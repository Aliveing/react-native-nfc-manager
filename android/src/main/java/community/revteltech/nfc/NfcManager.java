...
class NfcManager extends ReactContextBaseJavaModule implements ActivityEventListener, LifecycleEventListener {
  ...
  @ReactMethod
  public void writeNdefMessage(String string, Callback callback) {
    synchronized(this) {
      if (techRequest != null) {
        try {
          Ndef ndef = (Ndef)techRequest.getTechHandle();
          // use ndef record to transfer string
          ndef.writeNdefMessage(new NdefMessage(new NdefRecord[] {createTextRecord(string)}));
          callback.invoke();
        } catch (Exception ex) {
          ex.printStackTrace();
